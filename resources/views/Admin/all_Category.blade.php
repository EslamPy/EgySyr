            ...
            <tbody>
              @forelse ($all_Category as $Category)
                  <tr>
                    ...
                    <td>
                      <!-- زر التعديل -->
                      <a href="{{ route('Category.edit', $Category->id) }}" class="btn btn-block btn-outline-success btn-lg">تعديل</a>
                  
                      <!-- زر الحذف -->
                      <form action="{{ route('Category.destroy', $Category->id) }}" method="POST" style="display: inline-block; width: 100%;" onsubmit="return confirm('هل أنت متأكد أنك تريد حذف هذا المقال؟');">
                          @csrf
                          @method('DELETE')
